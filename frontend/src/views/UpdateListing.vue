<template>
  <div class="home">
    <h1 style="color: blueviolet; display: block">
      Edit a role listing (as a HR)
    </h1>
        ...
        <v-row>
          ...
          <v-col cols="8">
            <v-text-field
              variant="outlined"
              v-model="jobtitle"
              placeholder="Fill in the title of the Role Listing"
            >
            </v-text-field>
          </v-col>
        </v-row>
        ...
        <v-row>
          ...
          <v-col cols="8">
            <v-textarea
              ...
              placeholder="Provide additional description on the listing here"
            >
            </v-textarea>
          </v-col>
        </v-row>
          ...
          <v-col cols="8">
            <v-text-field
              ...
              :rules="[
                (v) =>
                  /^\d+$/.test(v) ||
                  'Please key in a positive whole number only.',
              ]"
              v-model="vacancies"
              type="number"
              placeholder="Please key in the number of vacancies"
            >
            </v-text-field>
          </v-col>
        </v-row>
  ...
  <v-dialog v-model="invalid" width="auto" id="error">
    <v-card>
      <v-card-title>
        <p style="text-align: center">Please Try Again</p>
      </v-card-title>
      <v-card-text id="errorMsg">
...
export default {
  ...
  methods: {
    ...
    getCurrListing() {
      axios
        .get("http://localhost:3003/listing/" + this.idparam)
        .then((response) => {
          ...
          this.open =
            this.responseHolder._open === 1 ? "Available" : "Unavailable";
        })
        .catch((error) => {
          console.log(error);
        });
      ...
      axios
        ...
        .then((response) => {
          ...
          for (var i = 0; i < this.responseHolder.body.length; i++) {
            ...
            if (
              this.roleslist.indexOf(this.responseHolder.body[i].role_name) !==
              -1
            ) {
              this.listtoskills[this.responseHolder.body[i].role_name].push(
                this.responseHolder.body[i].skill_name
              );
            } else {
              this.roleslist.push(this.responseHolder.body[i].role_name);
              this.listtoskills[this.responseHolder.body[i].role_name] = [
                this.responseHolder.body[i].skill_name,
              ];
            }
          }
    ...
    validation() {
      ...
      this.errormsg = [];
      if (
        this.isValid(this.jobtitle) == false ||
        this.isValid(this.rolename) == false ||
        this.isValid(this.jobdescription) == false ||
        this.isValid(this.dept) == false ||
        this.isValid(this.vacancies) == false ||
        this.isValid(this.country) == false ||
        this.isValid(this.expirydate) == false
      ) {
    ...
    submit() {
      ...
      if (this.validform == false) {
        ...
        axios
          .put("http://localhost:3003/listing/" + this.idparam, bodyInfo)
          .then((response) => {
            this.dialog = true;
          })
          .catch((error) => {
